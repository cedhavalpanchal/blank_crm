<?php

/*
  @Description: Reset Password
  @Author: Parag Joshi
  @Input:
  @Output:
  @Date: 25-11-2016

 */

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Reset_password_mobile_control extends CI_Controller {

    function __construct() {

        parent::__construct();
        $this->admin_session = $this->session->userdata($this->lang->line('badge_scanning_admin_session'));
        $this->message_session = $this->session->flashdata('message_session');
        $this->load->model('Common_function_model');
        $this->viewname = $this->router->uri->segments[2];
    }

    /*
      @Description: Function for Get All Template
      @Author: Parag Joshi
      @Input: - load templated
      @Output: -
      @Date: 25-11-2016
     */

    public function index() {

        $data['main_content'] = 'reset_password_mobile/' . $this->viewname . "/list";
        $this->load->view('reset_password_mobile/include/template', $data);
    }

    /*
      @Description: Function Load Reset Password Template
      @Author: Parag Joshi
      @Input: - Forgot Password
      @Output: - Send Email Template
      @Date: 25-11-2016
     */

    public function reset_password_template() {

        $token = $this->router->uri->segments[3];

        //$user_id = base64_decode(urldecode($id));
        if (!empty($token)) {
            
            $fields = array('id,CONCAT_WS(" ",first_name,last_name) as name,email,is_forgot_password,token');
            $match = array('token' => $token);
            $sq_data_all = array
                (
                    "table" => 'user_master',
                    "fields" => $fields,
                    "condition" => $match
                   );
            $exist_email = $this->Common_function_model->getmultiple_tables($sq_data_all);
            //prd($exist_email);

            if(!empty($exist_email))
            {
                $data['is_forgot_password'] = $exist_email[0]['is_forgot_password'];
                $data['main_content'] = "reset_password/reset_password_link/add_mobile";
                $this->load->view('reset_password/include/template', $data);
            }
            else
            {
                $msg = $this->lang->line('wrong_data_error');
                $status = $this->lang->line('message_type_failed');
            
                 $response = array(
                        "status" => $status,
                        "message" => $msg
                    );
                $this->session->set_flashdata('response', $response);
                redirect('admin');
            }
        }else
        {
            $msg = $this->lang->line('wrong_data_error');
            $status = $this->lang->line('message_type_failed');
            
            $response = array(
                    "status" => $status,
                    "message" => $msg
                );
            $this->session->set_flashdata('response', $response);
            redirect('admin');
        }
      
    }

    /*
      @Description: Function Load Reset Password
      @Author: Parag Joshi
      @Input: - Forgot Password in Add New password
      @Output: - login Admin/user
      @Date: 25-11-2016
     */

    public function change_password() {
        
        $token = $this->input->post('id');
        //$user_id  = base64_decode(urldecode($id));

        $password = $this->input->post('txt_npassword');

        $reset_pass['password']           = $this->Common_function_model->encrypt_script($password);
        $reset_pass['is_forgot_password'] = '0';
        //$reset_pass['token']              = '';
        //$reset_pass['id']                 = $user_id;
        $reset_pass['modified_date']      = date('Y-m-d: H-m-i');

        if (!empty($token)) {
            $fields = array('id,CONCAT_WS(" ",first_name,last_name) as name,email,token');
            $match = array('token' => $token);
            $sq_data_all = array
                (
                  "table" => 'user_master',
                  "fields" => $fields,
                  "condition" => $match
                 );
            $exist_email = $this->Common_function_model->getmultiple_tables($sq_data_all);
        } else {

            $msg = $this->lang->line('mail_not_registered');
            $status = $this->lang->line('message_type_failed');
            
            $response = array(
                    "status" => $status,
                    "message" => $msg
                );
            $this->session->set_flashdata('response', $response);
            redirect('reset_password_mobile/reset_password_template/'.$token);
            //redirect('admin');
        }
        
        if (!empty($exist_email)) {
            $this->Common_function_model->update('user_master',$reset_pass,array('id'=>$exist_email[0]['id']));
            
            $msg = $this->lang->line('password_change_succ');
            $status = $this->lang->line('message_type_success');
            $response = array(
                   "status" => $status,
                   "message" => $msg,
                   "is_expired" => '1'
               );
            
           $this->session->set_flashdata('response', $response);
           redirect('reset_password_mobile/reset_password_template/'.$token);
           
        } else {
            $msg = $this->lang->line('password_not_change');
            $status = $this->lang->line('message_type_failed');
            
            $response = array(
                    "status" => $status,
                    "message" => $msg,
                );
            $this->session->set_flashdata('response', $response);
            redirect('admin');
        }
    }
}
